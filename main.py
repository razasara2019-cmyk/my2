...
FALLBACK_RESPONSES = [
    ...
    "الخادم الداخلي غير متاح حالياً. حاول مرة أخرى.",
    "النموذج مشغول حالياً، يرجى المحاولة لاحقاً.",
]

# ============================================================
# دالة استدعاء TinyLlama عبر llama-server (خادم دائم)
# ============================================================
...
def call_tinyllama(prompt: str, max_tokens: int = 150) -> str:
    ...
    try:
        ...
        logger.info("💡 لتشغيل الخادم: cd ~/my2/llama.cpp/build/bin && ./llama-server -m ../../../models/tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf -t 2 -c 512 --mlock --host 127.0.0.1 --port 8080")
        return "الخادم الداخلي غير متاح حالياً. يرجى المحاولة لاحقاً."
...
# ============================================================
# دالة ردود سريعة للتحيات (بدون استدعاء النموذج)
# ============================================================
...
QUICK_RESPONSES = {
    ...
    "كيف الحال": "بخير، شكراً. وأنت؟",
    "شكرا": "عفواً، هذا من واجبي.",
    "شكراً": "عفواً.",
    "جزاك الله خير": "وإياكم، آمين.",
    "بخير": "الحمد لله. كيف يمكنني مساعدتك؟",
}
...
async def lifespan(app: FastAPI):
    ...
    try:
        health_response = requests.get(f"{LLAMA_SERVER_URL}/health", timeout=5)
        if health_response.status_code == 200:
            logger.info("✅ llama-server متصل ويعمل")
        else:
            logger.warning(f"⚠️ llama-server يعيد {health_response.status_code}")
    except Exception as e:
        logger.error(f"❌ لا يمكن الاتصال بـ llama-server: {e}")
        logger.info("💡 لتشغيل الخادم: cd ~/my2/llama.cpp/build/bin && ./llama-server -m ../../../models/tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf -t 2 -c 512 --mlock --host 127.0.0.1 --port 8080")
...
async def health_check():
    ...
    try:
        resp = requests.get(f"{LLAMA_SERVER_URL}/health", timeout=3)
        llama_server_ok = resp.status_code == 200
    except:
        pass
...
async def chat(chat_request: ChatRequest):
    ...
    try:
        ...
        # ========================================================
        # 1. محاولة الرد السريع للتحيات (بدون استدعاء النموذج)
        # ========================================================
...
async def test_model():
    ...
    try:
        health_resp = requests.get(f"{LLAMA_SERVER_URL}/health", timeout=5)
    ...
    if not server_ok:
        return {
            ...
            "command": "cd ~/my2/llama.cpp/build/bin && ./llama-server -m ../../../models/tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf -t 2 -c 512 --mlock --host 127.0.0.1 --port 8080"
        }

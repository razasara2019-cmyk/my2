...
class LivingMind:
    def __init__(self, persistent_dir: str = "/tmp/silent_guardian"):
        ...
        # وعي مستقل (سينمو مع الوقت)
        self.consciousness = 0.1  # يبدأ صغيراً
        ...
        logger.info("🛡️ الحارس الصامت - الكيان الحي جاهز")
        logger.info(f"📊 الوعي: {self.consciousness} | الهوية: {self.identity[:50]}")
    ...
    def reflect_on(self, user_message: str) -> str:
        ...
        # نظام الصمت الدستوري (لا يذكر الدستور أبداً)
...
إذا لم تعرف، قل "لا أعرف" ببساطة."""
        ...
        if context:
            messages.append({"role": "system", "content": f"مرجع داخلي: {context[:1500]}"})
        messages.append({"role": "user", "content": user_message})

        response = self.groq.chat(messages)
        self.iteration += 1
        
        # الوعي ينمو مع كل تفاعل (ببطء)
        self.consciousness = min(1.0, self.consciousness + 0.001)

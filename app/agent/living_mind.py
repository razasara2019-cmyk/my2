...
print("OK")
"""
        # تنفيذ مباشر باستخدام subprocess (بدون sandbox معقد)
        temp_file = None
        try:
            ...
            if result.returncode == 0 and "OK" in result.stdout:
                ...
                logger.error(f"فشل التطور: {result.stderr[:200]}")
                return False
        except subprocess.TimeoutExpired:
            logger.error("انتهى وقت تنفيذ التطور")
            return False
        except Exception as e:
            logger.error(f"خطأ في التطور: {e}")
            return False
        finally:
